...
export const generateRoundingsTableDef = (timeSeries: TimeSeriesElement) => {
  const { varName: sourceVarName } = timeSeries;
  const [, headerRow] = timeSeries.children;
  const roundingsTableBlockId = `${timeSeries.id}-roundings`;

  return statementToIdentifiedBlock(roundingsTableBlockId, {
    type: 'table',
    args: [
      {
        type: 'tabledef',
