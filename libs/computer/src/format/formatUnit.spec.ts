...
describe('base unit tests', () => {
  it('runs a single test', () => {
    expect(
      formatUnit(locale, U([u('meter'), u('kg'), Unit.inverseExponent(u('s'))]))
    ).toEqual('meters·kg/s');
  });
  ...
  it('can follow SI units rules and style conventions', () => {
    ...
    expect(
      formatUnit(locale, U([meter, Unit.inverseExponent(second)]))
    ).toEqual('meters per second');
    expect(
      formatUnit(locale, U([meter, Unit.inverseExponent(second)]))
    ).toEqual('meters per second');
    expect(
      formatUnit(locale, U([u('meters'), Unit.inverseExponent(u('s'))]))
    ).toEqual('meters/s');
    ...
    expect(
      formatUnit(
        locale,
        U([u('meters'), u('kg'), Unit.inverseExponent(u('s'))])
      )
    ).toEqual('meters·kg/s');
  ...
  it('formats simply', () => {
    expect(
      simpleFormatUnit(U([u('m'), u('kg'), Unit.inverseExponent(u('s'))]))
    ).toEqual('m * kg * s^-1');
  });
});
