...
const drawEdges = (node: Node, allNodes: EntityNodeMap): void => {
  const deps = dependencies(node.value.block);
...
export const topologicalSort = (blocks: ParseRet[]): ParseRet[] => {
  ...
  const nodes = blocks.filter(goodBlock).map(blockToNode);
  ...
  for (const node of nodes) {
    drawEdges(node, identifiersToNode);
  }
