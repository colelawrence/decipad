...
import { normalizePlainTextChildren } from '../utils';
import {
  NormalizerReturnValue,
  createNormalizerPlugin,
} from '../../normalizerPlugin';

const assertElementType = (
  node: TElement
): node is CodeLineV2Element | DataTabChildrenElement => {
  return (
    node.type === ELEMENT_CODE_LINE_V2 ||
    node.type === ELEMENT_DATA_TAB_CHILDREN
  );
};
