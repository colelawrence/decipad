...
describe('pads', () => {
  ...
  test('adding a pad with tags gets indexed', async () => {
    ...
    deci.workspace(workspaceId).pads.byTag.get('tag 1').subscribe(({ loading, data: pads }) => {
      if (!loading) {
  ...
  test('adding another pad with a common tag also gets indexed', async () => {
    ...
    deci.workspace(workspaceId).pads.byTag.get('tag 2').subscribe(({ loading, data: pads }) => {
      if (!loading) {
  ...
  test('updating a pad updates the tag index', async (done) => {
    ...
    deci.workspace(workspaceId).pads.byTag.get('tag 2').subscribe(({ loading, data: pads }) => {
      if (!loading) {
        ...
        deci.workspace(workspaceId).pads.byTag.get('tag 3').subscribe(({ loading, data: pads }) => {
          if (!loading) {
            expect(pads).toHaveLength(1)
            expect(pads![0]).toBe('pad id 2')
            deci.stop()
