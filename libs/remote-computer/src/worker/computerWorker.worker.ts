  ...
  const initialized = new Promise<void>((initializationDone) => {
    ...
    >['subscribe'] = (options, notify) => {
      ...
      initialized
        ...
        .catch((err) => {
          console.error('Error initializing remote computer worker', err);
        });
      return () => {
        unsubscribed = true;
      ...
      async ([req]) =>
        queue.push(async () => {
          ...
          return computer.pushComputeDelta(
            await decodeComputeDeltaRequest(req)
          );
        })
    );
    ...
    (async () => {
      ...
      await rpc
        .call('computerReady', [])
        .catch((err) => {
          console.error('Error notifying computer ready', err);
        })
        .then(() => {
          console.log('Signaled computer worker ready');
        });
    })();

    const firstResultsDone = new Promise<void>((resolve) => {
      const sub = computer.results.subscribe(() => {
        setTimeout(() => {
          sub.unsubscribe();
        });
        resolve();
