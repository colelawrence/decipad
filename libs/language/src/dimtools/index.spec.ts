...
describe('reduceTypesThroughDims', () => {
  ...
  it('allows the same tuples', () => {
    ...
    expect(reduceTypesThroughDims([type, otherType], typeId)).toEqual(type);
  });

  it('errors with incompatible dims', () => {
    const errored1 = reduceTypesThroughDims(
      [num, Type.buildTuple([num, str])],
      typeId
    );
    ...
    const notErrored = reduceTypesThroughDims(
      [Type.buildColumn(num, 2), Type.buildTuple([num, str])],
      typeId
    );
  ...
  it('errors with tuples with different cell names', () => {
    ...
    expect(reduceTypesThroughDims([t1, tDiffNames], typeId)).toEqual(
      Type.Impossible.withErrorCause('Mismatched tuples')
    );

    expect(reduceTypesThroughDims([t1, tNoNames], typeId)).toEqual(
      Type.Impossible.withErrorCause('Mismatched tuples')
    );
  });
