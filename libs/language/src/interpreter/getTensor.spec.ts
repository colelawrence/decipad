...
it('can perform calculations between columns and single numbers', () => {
  expect(testGetTensor(c('*', col(1, 2, 3), l(2)))).toEqual([2, 4, 6]);

  expect(testGetTensor(c('/', col(1, 2, 3), l(2)))).toEqual([0.5, 1, 1.5]);

  expect(testGetTensor(c('+', l(1), col(1, 2, 3)))).toEqual([2, 3, 4]);
});
