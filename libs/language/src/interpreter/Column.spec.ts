...
`);
  ...
  it('a table can be sliced', () => {
    const col1 = Column.fromValues([1, 2, 3, 4, 5, 6, 7, 8, 9].map(fromJS));
    const col2 = Column.fromValues(
      ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'].map(fromJS)
    );
    const table = Column.fromNamedValues([col1, col2], ['col1', 'col2']);
    const slice1 = table.sliceEach(3, 6);
    const slice2 = table.sliceEach(7, 9);
    expect(table.getData()).toMatchInlineSnapshot(`
      Array [
        Array [
          Fraction(1),
          Fraction(2),
