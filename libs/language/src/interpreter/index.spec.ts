...
describe('interpreter', () => {
  ...
  it('evaluates and returns', async () => {
    ...
    expect(await run(basicProgram, ['A'])).toEqual([[N(42), undefined]]);
  });

  it('Gets specific statement', async () => {
    const basicProgram = [n('block', c('+', l(1), l(1)))];

    expect(await run(basicProgram, [[0, 0]])).toEqual([[N(2), undefined]]);
  });
  ...
  describe('sequences', () => {
    it('can be evaluated', async () => {
      ...
      expect(
        await materializeOneResult(runOne(seq(l(5), l(1), l(-1))))
      ).toEqual([N(5), N(4), N(3), N(2), N(1)]);
  ...
  describe('functions', () => {
    it('can create and use functions', async () => {
      ...
      expect(
        await materializeOneResult(interpret([usingFunctions], [0]))
      ).toEqual([N(3)]);
    });
  });
  ...
  it('Can use variables', async () => {
    ...
    expect(await materializeOneResult(interpret([withVariables], [0]))).toEqual(
      [N(1)]
    );
  });
  ...
  describe('Tables', () => {
    ...
    it('can get a column from a table', async () => {
      ...
      expect(await materializeOneResult(interpret([block], [0]))).toEqual([
        ['hi', 'there'],
      ]);
    });
  ...
  describe('higher dimensions', () => {
    it('Can operate upon 2D columns', async () => {
      ...
      expect(
        await materializeOneResult(runOne(c('+', column, column)))
      ).toEqual([
        [N(2), N(4), N(6)],
        [N(8), N(10), N(12)],
      ]);
