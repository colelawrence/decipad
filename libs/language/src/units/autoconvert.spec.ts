...
describe('autoconvert arguments', () => {
  it('autoconverts unitless argument', async () => {
    expect(
      await autoconvertArguments([fromJS(N(2))], [number()])
    ).toMatchObject([fromJS(N(2))]);
  });

  it('autoconverts unitful (standard unit) argument', async () => {
    expect(
      await autoconvertArguments([fromJS(N(2))], [number(U('meter'))])
    ).toMatchObject([fromJS(N(2))]);
  });
  ...
  it('autoconverts expandable (but standard) units (2)', async () => {
    // 2 bar
    expect(
      await autoconvertArguments([fromJS(2)], [number(U('bar'))])
    ).toMatchObject([fromJS(200_000_000)]);
  });

  it('autoconverts non-scalar units', async () => {
    expect(
      await autoconvertArguments([fromJS(2)], [number(U('celsius'))])
...
describe('autoconvert result', () => {
  it('autoconverts unitless result', async () => {
    expect(await autoconvertResult(fromJS(N(2)), number())).toMatchObject(
      fromJS(N(2))
    );
  });
  ...
  it('autoconverts unitful (standard unit) argument', async () => {
    expect(
      await autoconvertResult(fromJS(N(2)), number(U('meter')))
    ).toMatchObject(fromJS(N(2)));
  });

  it('autoconverts from expandable (but standard) units (2)', async () => {
    expect(
      await autoconvertResult(fromJS(200_000_000), number(U('bar')))
    ).toMatchObject(fromJS(N(2)));
  });

  it('autoconverts non-scalar units', async () => {
    expect(
      await autoconvertResult(fromJS(N(27515, 100)), number(U('celsius')))
    ).toMatchObject(fromJS(N(2)));
  });
});
