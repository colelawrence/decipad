...
export const mutateAst = (
  ...
  const newNode = fn(_node, path);
  for (let index = 0; index < newNode.args.length; index++) {
    const arg = newNode.args[index];
    if (isNode(arg)) {
      const newSubNode = mutateAst(arg, fn, [...path, index]);
      newNode.args[index] = newSubNode;
    }
  }
