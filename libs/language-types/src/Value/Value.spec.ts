...
import { parseUTCDate } from '../Time/parseUTCDate';
...
describe('Value', () => {
  ...
  it('can get from JS for testing', () => {
    ...
    expect(omit(fromJS([1, 2, 3]), ['meta'])).toEqual(
      omit(
        Column.fromValues(
          [Scalar.fromValue(1), Scalar.fromValue(2), Scalar.fromValue(3)],
          emptyMeta
        ),
        ['meta']
      )
  ...
  it('can create column from generator', async () => {
    const col = Column.fromGenerator(
