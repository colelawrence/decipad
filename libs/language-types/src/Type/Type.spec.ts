...
describe('divideUnit', () => {
  ...
  it('exponentiates units', async () => {
    expect(await t.number([invSecond]).divideUnit([second])).toEqual(
      t.number([Unit.setExponent(second, N(-2))])
    );

    expect(
      await t.number([Unit.setExponent(second, N(-2))]).divideUnit([second])
    ).toEqual(t.number([Unit.setExponent(second, N(-3))]));
  });
  ...
  it('divides units even further', async () => {
    expect(
      ...
      Unit.setExponent(u('USD'), N(-1)), // first because of sort
      meter,
      invSecond,
    ]);
  });
