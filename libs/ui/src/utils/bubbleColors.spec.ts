...
describe('bubbleColors', () => {
  Object.keys(expectedResults).forEach((_colorName) => {
    const colorName = _colorName as keyof typeof expectedResults;
    const { highlighted, normal } =
      expectedResults[colorName as keyof typeof expectedResults];
