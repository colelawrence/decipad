...
export const StarterChecklist: FC<StarterChecklistProps> = ({
  ...
  const [hide, setHide] = useState(checklist.completed);
    ...
    <div css={[wrapperStyles, hide && { height: COLLAPSED_HEIGHT }]}>
      <div css={innerStyles}>
        ...
        {hide ? (
          <div>
            <p>
              {Object.values(checklist.items).find((v) => !v.state)
                ? `Next up - ${
                    Object.values(checklist.items).find((v) => !v.state)?.text
                  }`
                : 'All done!'}
            </p>
            <Progress
          ...
          <>
            <div css={checklistStyles}>
              ...
              <div css={[checklistStyles, { gap: 8 }]}>
                {Object.values(checklist.items).map((v) => (
                  <div
                    key={nanoid()}
                    css={{
                      display: 'flex',
