import { Box, Divider } from '@chakra-ui/react';
import React from 'react';
import { RenderElementProps } from 'slate-react';
import { Blockquote } from './Blockquote.component';
import { BulletList } from './BulletList.component';
import { Code } from './Code.component';
import { CodeBlock } from './CodeBlock.component';
import {
  HeaderFive,
  HeaderFour,
  HeaderOne,
  HeaderSix,
  HeaderThree,
  HeaderTwo,
} from './Headers';
import { ListItem } from './ListItem.component';
import { Mention } from './Mention.component';
import { NumberedList } from './NumberedList.component';
import { Paragraph } from './Paragraph.component';
import { TodoListItem } from './TodoListItem.component';

export const Elements = (props: RenderElementProps): JSX.Element | null => {
  switch (props.element.type) {
    case 'h1':
      return <HeaderOne {...props} />;
    case 'h2':
      return <HeaderTwo {...props} />;
    case 'h3':
      return <HeaderThree {...props} />;
    case 'h4':
      return <HeaderFour {...props} />;
    case 'h5':
      return <HeaderFive {...props} />;
    case 'h6':
      return <HeaderSix {...props} />;
    case 'blockquote':
      return <Blockquote {...props} />;
    case 'p':
      return <Paragraph {...props} />;
    case 'code':
      return <Code {...props} />;
    case 'code_block':
      return <CodeBlock {...props} />;
    case 'ul':
      return <BulletList {...props} />;
    case 'ol':
      return <NumberedList {...props} />;
    case 'li':
      return <ListItem {...props} />;
    case 'action_item':
      return <TodoListItem {...props} />;
    case 'divider':
      return (
        <Box {...props.attributes}>
          <Divider my="5" contentEditable={false} />
          {props.children}
        </Box>
      );
    case 'mention':
      return <Mention {...props} />;
    default:
      return <Box {...props} />;
  }
};
