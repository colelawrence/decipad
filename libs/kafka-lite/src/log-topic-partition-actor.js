...
async function createLogTopicPartitionActor(conf, topic, partition) {
  ...
  async function indexPartitionSegments() {
    ...
    for (const fileName of fileNames) {
      ...
      assert(
        nameParts.length === 2 &&
          (nameParts[1] === 'index' || nameParts[1] === 'log'),
    ...
    if (segmentOffsetStarts.length) {
      const lastFileOffset =
        segmentOffsetStarts[segmentOffsetStarts.length - 1];
  ...
  async function fetch(offset, maxBytes, maxWaitMs) {
    ...
    try {
      ...
      bytesRead = (await indexFileHandle.read(indexContents, 0, 16, fileOffset))
        .bytesRead;
    ...
    if (bytesRead === 16) {
      ...
      assert(
        readOffset === offset,
        'stored offset is different from requested offset'
      );
      ...
      const { bytesRead: logBytesRead } = await logFileHandle.read(
        logContents,
        0,
        logContents.length,
        logFilePos
      ...
      while (remainder.length >= requiredBytes) {
        ...
        if (remainder.length >= recordLength) {
          ...
          assert(
            decodedOffset === expectedOffset,
            `saved offset is ${decodedOffset} but expected ${expectedOffset}`
          );
