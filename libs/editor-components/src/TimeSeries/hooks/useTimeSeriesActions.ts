...
export const useTimeSeriesActions = (
  editor: MyEditor,
  element: TimeSeriesElement | undefined
): TableActions => {
  ...
  const setDataColumns = useCallback(
    (columns: Column[]) => {
      ...
      const headerRow: TimeSeriesHeaderRowElement | undefined =
        element?.children[1];
      const headerRowPath = headerRow && findNodePath(editor, headerRow);
      if (!headerRowPath) {
        return;
      }
      const existingColumns: TimeSeriesHeader[] | undefined =
  ...
  const onDeleteColumn = useCallback(
    (timeSeriesHeaderPath: Path) => {
      removeNodes(editor, { at: timeSeriesHeaderPath });
    },
    [editor]
  );
