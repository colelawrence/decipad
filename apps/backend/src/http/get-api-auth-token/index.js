...
const TOKEN_COOKIE_NAMES = ['next-auth.session-token', '__Secure-next-auth.session-token']

exports.handler = handle(async (event) => {
  const cookies = parseCookies(event.cookies)
  const token = cookies[TOKEN_COOKIE_NAMES[0]] || cookies[TOKEN_COOKIE_NAMES[1]]
  let user = null
  if (token) {
    try {
