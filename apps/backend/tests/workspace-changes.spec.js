...
async function createClient(userId) {
  ...
  const link = createWebsocketLink(createDeciWebsocket({ token }), 15000);
  return withAuth({ token, link });
}

async function subscribe(userId, workspaces, subscriptions) {
      ...
      next({ data }) {
        ...
        if (changes.updated) {
          for (const w of changes.updated) {
            const index = workspaces.findIndex((w2) => w2.id === w.id);
