...
export const handler = handle(async (event: APIGatewayProxyEvent): Promise<HttpResponse | string> => {
  const { user, secret } = await auth(event);
...
async function generateToken(secret: string, options: Partial<JWTEncodeParams>): Promise<string> {
