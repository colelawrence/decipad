...
env -i PATH="${PATH:-}" LANG="${LANG:-}" PWD="${PWD:-}" HOSTNAME="${HOSTNAME:-}" USER="${USER:-}" UID="${UID:-}" SHELL="${SHELL:-}" TMPDIR="${TMPDIR:-}" LD_LIBRARY_PATH="${LD_LIBRARY_PATH:-}" npx yarn build:frontend
